...
def save_code(code, file_path):
    ...
    df = pd.read_csv(file_path)
    df = pd.concat([df, pd.DataFrame([{'code': code, 'timestamp': timestamp}])], ignore_index=True)
...
def save_cups_data(code, number_of_cups):
    ...
    df = pd.DataFrame(df)
    df = pd.concat([df, pd.DataFrame([{'code': code, 'number_of_cups': number_of_cups, 'timestamp': timestamp}])], ignore_index=True)
